...
#define RSA_KEY_BITS 2048     // RSA密钥长度（2048/4096）

// 全局缓存：内存生成的RSA私钥（全程不落地）
...
static int generate_rsa_key_in_memory() {
    if (g_rsa_pkey) return 0; // 已生成，直接返回

    // 1. 初始化RSA密钥生成上下文
    EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new_id(EVP_PKEY_RSA, NULL);
    if (!ctx) {
        fprintf(stderr, "EVP_PKEY_CTX创建失败\n");
        return -1;
    }

    // 2. 初始化密钥生成
    if (EVP_PKEY_keygen_init(ctx) != 1) {
        fprintf(stderr, "密钥生成初始化失败\n");
        EVP_PKEY_CTX_free(ctx);
        return -1;
    }

    // 3. 设置密钥长度（2048/4096）
    if (EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, RSA_KEY_BITS) != 1) {
    ...
    // 5. 清理上下文
    EVP_PKEY_CTX_free(ctx);
    printf("✅ 内存中生成RSA-%d密钥对成功（未落地存储）\n", RSA_KEY_BITS);
    return 0;
}

// RSA-SHA256签名（使用内存中的私钥，OpenSSL 3.x写法）
static char* rsa_sign(const char *data) {
    if (!g_rsa_pkey) {
        fprintf(stderr, "RSA私钥未初始化\n");
        return NULL;
    }
    ...
    if(EVP_DigestSignInit(ctx, NULL, EVP_sha256(), NULL, g_rsa_pkey) != 1 ||
...
// 生成JWT（ID Token/Access Token）- 使用内存私钥
...
int main() {
    ...
    if (generate_rsa_key_in_memory() != 0) {
        fprintf(stderr, "❌ 内存生成RSA密钥失败，程序退出\n");
        return 1;
    }
    ...
    free(auth_codes);
    if (g_rsa_pkey) EVP_PKEY_free(g_rsa_pkey); // 释放内存私钥
    return 0;
}
